...
Claude Code blocks user/project memory with `--setting-sources local`, and the
OpenCode SDK redirects `XDG_CONFIG_HOME` to block user/global instruction
layers while leaving project `AGENTS.md` active. OpenCode CLI has no blocking
profile (`--pure` is plugins-only); Cursor User Rules remain injected
(ADR 0045).
